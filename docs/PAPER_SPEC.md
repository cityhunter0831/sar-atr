...
2. **대비 자동조절 (Optuna 하이퍼파라미터 자동탐색)** (Exp C) — 논문은 대비 레벨 3개를 근거 없이 임의 고정. 우리는 Optuna로 대비 파라미터를 자동 최적화(재현성·객관성). 논문 3레벨 재현을 baseline으로 유지. ✅ 구현됨
3. **XAI × 산란점 IoU 검증** — 모델이 물리적 산란점을 보는지 (Exp B). Grad-CAM→픽셀 단위 XAI(Occlusion/SmoothGrad-IG)로 확장. ✅ 구현됨
