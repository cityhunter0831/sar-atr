...
| 로그진폭(dB) 전처리 도입 | few-shot 정확도 대폭 개선 |
...
- **Baseline**: few-shot 난이도 정확 재현 (136장 → 55.9%, 논문 56.6%와 거의 동일)
- **증강 효과**: +32.6%p 개선으로 물리 기반 증강의 효과 입증
- **잔여 갭 (7.9%p)**: tracked vehicle(T72·2S1) 병목 — 클래스별 sigma_G 미세조정 미완
...
| 조건 | 우리 결과 (RN18, K=0) |
|---|---|
| ① 증강 없음 (synth→real) | 81.5% |
| ② 논문 ColorJitter(0.5) x 3 | 61.8% (**-19.7%p 악화**) |
| ③ Optuna (strength=0.317, levels=4) | **75.4%** (**+13.6%p 개선**) |

**핵심 발견**: 논문의 매직넘버 0.5가 이 데이터에서는 오히려 **성능을 크게 해침** (81.5 → 61.8%). Optuna가 찾은 최적값(0.317, 4레벨)으로 **61.8 → 75.4%** 회복.

**발표 포인트**: "근거 없는 임의 대비값은 데이터에 따라 독이 될 수 있고, 자동탐색이 근거 있는 최적을 찾는다."

**절대 수치 갭 (81.5 vs 논문 91.9) 설명**: SAMPLE 로드 장수(train/test 각 1345)가 논문 Table 5(806/539)와 달라 서브셋 규약 차이. 개선 서사(+13.6%p)와는 무관.
...
- **Near-OOD**: Maha AUROC 0.356~0.456 (랜덤 수준). 동일 도메인 내 미지 클래스는 현재 방법으로 탐지 불가.
...
| 2 | **Optuna 대비 자동탐색** | contrast=0.5, 3레벨 매직넘버 | 베이지안 최적화로 근거 있는 최적값 | +13.6%p |
...
| `exp_b_confusion_matrix.png` | Exp B | Baseline(55.9%) vs Aug(88.5%) 혼동행렬 나란히 | Exp B 재현 결과 |
| `exp_b_xai_iou.png` | Exp B | IoU 단조증가 막대 + 방법 비교표 | 개선 #3 결과 |
| `exp_c_contrast_optuna.png` | Exp C | 3단계 비교 막대 + Optuna 파라미터 비교 테이블 | Exp C + 개선 #2 결과 |
...
- "136장 → 55.9% (few-shot, 논문 56.6%와 동일) → 증강 후 88.5% (논문 96.4%에 근접)"
- 하이브리드 파이프라인 다이어그램
...
- "논문 0.5가 오히려 독 (-19.7%p) → Optuna로 +13.6%p 회복"
...
- 총괄 결과표 (4실험 x 판정)
- 개선 3종 요약
- 한계: Exp B 잔여 갭 7.9%p (T72/2S1), Exp C 절대 수치 갭 (서브셋 규약 차이)
