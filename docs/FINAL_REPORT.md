...
| **Exp D** | OOD 탐지 | 학습에 없던 표적을 "모른다"고 걸러낼 수 있는가? | SAMPLE + MSTAR-O + SAR-ship |
...
| Exp C (Table 6, RN18) | K=0: 91.9%→94.5%, K=0.1: 97.8%→98.9% | K=0: 63.8%→**87.7%**, K=0.1: 95.7%→**96.9%** — 전 K에서 Aug>Ori | 방향성 재현 |
| Exp D (Fig 9) | ID 학습 K=0.1, near/far-OOD AUROC 비교 | ID 정확도 **91~96%**, ODIN/Maha AUROC **0.79~0.96** (holdout·MSTAR-O·SAR-ship 전 구간) | 재현(seed 1개 검증, 확장 진행 중) |

> Exp C/D는 이번 세션에서 **paper-faithful 재설계**(복소 `.mat` 3단계 대비 렌더링, K=0/0.05/0.1 real-synth 혼합, 논문이 지정한 holdout 클래스, MSTAR-O far-OOD 추가)를 거쳐 완전히 새로운 파이프라인으로 재현했다 — 아래 §5·§6 수치는 이 새 파이프라인 기준(구버전 ColorJitter/SAR-ship-only 결과 아님).
...
### 5.2 논문 충실 재현 결과 (2026-07 Colab 검증)

**재현 방법**: SAMPLE 데이터셋이 원래 배포하는 **복소 원본 `.mat`**(`mat_files/{real,synth}`)에서 직접 Original/Contrast1/Contrast2 3단계 대비를 log-dB 압축으로 렌더링(논문 Figure 8의 "복소 이미지에서 다중 대비 레벨을 생성한다"는 서술을 그대로 따름). 이전에 사용하던 사후 렌더링 PNG에 `ColorJitter`를 씌우는 방식(§5.4 참조)과는 **완전히 다른, 논문 원문 그대로의 파이프라인**이다. 논문이 지정한 K(학습셋 중 실측 비율) 3단계 — K=0(100% 합성) / K=0.05 / K=0.1 — 로 RN18을 학습해 비교했다.

| K | 우리 Ori | 우리 Aug | 논문(RN18) Ori→Aug |
|---|---|---|---|
| 0.0 | 63.8% | **87.7%** (+23.9%p) | 91.9%→94.5% (+2.6%p) |
| 0.05 | 93.8% | **94.4%** (+0.6%p) | (미보고) |
| 0.1 | 95.7% | **96.9%** (+1.2%p) | 97.8%→98.9% (+1.1%p) |

**모든 K에서 Aug > Ori — 논문의 핵심 주장(대비 증강이 synth→real 갭을 줄인다)이 방향성 그대로 재현됨.** K=0.05/0.1에서는 개선폭(+0.6~1.2%p)이 논문(+1.1%p)과 비슷한 규모이고, K=0에서만 격차가 크게 벌어지는데(+23.9%p vs 논문 +2.6%p) 이는 우리 Ori 베이스라인(63.8%)이 논문(91.9%)보다 낮게 나온 결과다 — seed 1개 검증이라 분산은 아직 미확인이며, seeds=[0,1,2]×4모델 확장이 진행 중이다.

### 5.3 우리의 별도 탐색 — 개선 #2: Optuna 자동탐색 (2단 구조)
...
> ⚠️ 아래 5.4~5.6은 §5.2의 논문 충실 재현(.mat 기반, K=0/0.05/0.1)과는 **별개의 초기 탐색**으로, 사후 렌더링 PNG에 ColorJitter를 씌우는 구버전 파이프라인 기준 수치다. 매직넘버(contrast=0.5)의 위험성과 Optuna 자동탐색의 효과라는 방법론적 기여(개선 #2)는 그대로 유효하지만, 절대 수치는 §5.2의 최신 재현 결과로 대체된 것이니 발표 시 이 점을 구분해서 설명할 것.

### 5.4 그래프 해석 — `exp_c_contrast_optuna.png`
...
### 5.5 결과 비교표 (구버전 ColorJitter 파이프라인)
...
> ⚠️ 아래 이미지는 구버전 수치(SAR-ship만, holdout 클래스 오류) 기준으로 생성된 것 — §6.2 표가 최신 검증 수치이며, 그래프는 노트북 Cell 9vis 재실행으로 갱신 필요(발표 전 시간 부족 시 §6.2 표를 슬라이드에 직접 사용).
...
| **Far-OOD** | 완전히 다른 도메인 | MSTAR-O(다른 5클래스 군용 차량), 선박(SAR-ship) | 상대적으로 쉬움 |

**두 탐지 방법 비교**:

| 방법 | 원리 | 강점 | 약점 |
|---|---|---|---|
| **ODIN** | softmax 신뢰도에 temperature(T=1000) + 입력 섭동(ε=0.0014) 적용 → ID는 더 자신, OOD는 덜 자신하게 | 구현 간단, 사전 학습 불필요 | 입력 정규화 방식에 민감(아래 버그 참조) |
...
**실험 조건 (논문 원문 페이지 이미지 대조로 확정, paper-faithful 재설계)**:
- **ID = SAMPLE 10클래스**, K=0.1(실측 10%+합성 90% 혼합) + 3단계 대비 증강으로 학습
- **Near-OOD = holdout** — 논문 Figure 11/Figure 10 캡션 대조로 확정한 J=1/2/3: J=1={M1}, J=2={M35,M548}, J=3={M1,M35,M548}
- **Far-OOD = MSTAR-O**(BRDM2/BTR60/D7/T62/ZIL131, 논문 Figure 9가 실제로 비교하는 3종 중 하나) + **SAR-ship**(우리가 추가한 보조 far-OOD — 논문은 MiniSAR+SAR-ship을 OE 학습 재료로만 쓰고 Figure 9의 OOD 테스트셋으로는 Holdout/MSTAR-O/MSTAR-P를 씀. MiniSAR은 비공개라 대체 불가, MSTAR-P는 우선순위 낮게 보류)

### 6.2 결과 (2026-07 Colab 검증, seed=0)

| J (holdout) | ID 정확도 | ODIN holdout | ODIN MSTAR-O | ODIN sarship | Maha holdout | Maha MSTAR-O | Maha sarship |
|---|---|---|---|---|---|---|---|
| 1 ({M1}) | **95.8%** | 0.928 | 0.901 | 0.962 | 0.825 | 0.941 | 0.899 |
| 2 ({M35,M548}) | **91.3%** | 0.835 | 0.890 | 0.956 | 0.832 | 0.895 | 0.943 |
| 3 ({M1,M35,M548}) | **92.4%** | 0.860 | 0.857 | 0.946 | 0.792 | 0.951 | 0.892 |

### 6.3 이 결과가 의미하는 것

1. **K=0.1(실측 10%) + 대비 증강이 핵심적이었다**: 이전엔 ID 학습을 K=0(100% 합성)으로 돌려 ID 정확도가 21~29%에 머물렀다. 논문 Figure 13이 "K=0은 가장 나쁜 조건"이라 명시한 대로, K=0.1로 바꾸자 ID 정확도가 **91~96%로 급등**했다 — 논문이 K=0.1을 헤드라인 설정으로 쓴 이유를 실측으로 확인한 셈이다.

2. **Near-OOD(holdout)가 far-OOD보다 대체로 어렵다** (논문 Figure 9 패턴과 일치): 예를 들어 J=1 ODIN에서 holdout(0.928) < sarship(0.962), J=3에서 holdout(0.860) < MSTAR-O(0.857)·sarship(0.946)과 비슷하거나 낮음. 같은 SAMPLE 군사 차량 도메인 내 미지 클래스(near-OOD)는 학습 클래스와 반사 패턴이 유사해 구분이 더 어렵고, MSTAR-O(다른 데이터셋의 군용 차량)나 SAR-ship(완전히 다른 선박 도메인)은 상대적으로 구분이 쉽다는 논문의 일반적 직관과 일치한다.

3. **ODIN·Mahalanobis 둘 다 전 구간에서 안정적으로 작동**: 이전 버전에서 ODIN이 far-OOD(mstar_o/sarship)에서 예외 없이 AUROC=0.000(완전 역전)이 나오는 버그가 있었는데, 원인은 far-OOD 데이터(raw MSTAR 진폭)를 ID(SAMPLE)와 다른 정규화(선형)로 렌더링해 극단적인 도메인 시프트가 생긴 것이었다. ID와 동일한 log-dB 압축 정규화로 통일하자 ODIN도 0.83~0.96의 정상 범위로 회복됐다 — **OOD 탐지에서 ID/OOD 입력의 전처리 일관성이 탐지기 자체 알고리즘만큼 중요하다**는 실증적 교훈이다.

**한계 및 진행 중인 확장**: 위 수치는 seed=0 1회 검증 결과다. 논문처럼 분산(±std)을 보고하려면 seeds=[0,1,2] 확장이 필요하며, 이는 진행 중이다(Colab 무료 런타임의 반복적 연결 끊김으로 현재 중단).

**발표 포인트**: "ID 학습 설정(K=0.1, 대비 증강)과 ID/OOD 간 정규화 일관성이라는 두 가지 파이프라인 디테일이 OOD 탐지 성능을 21%→96%, AUROC 0.000→0.96으로 뒤바꿨다. 이는 OOD 탐지 알고리즘 자체보다 **데이터 파이프라인의 정합성**이 실전 성능을 좌우할 수 있음을 보여준다."
...
| `exp_d_ood.png` | Exp D | AUROC 히트맵 (⚠️ 재생성 필요, §6.2 표 참조) | near-OOD(holdout)가 far-OOD(MSTAR-O/SAR-ship)보다 대체로 어려움 |
...
| Exp C | mat 기반 3단계 대비, K=0/0.05/0.1 | 동일 + Optuna 자동탐색 병행 | 논문 그대로 재현 + 매직넘버 대안 제시 (개선 #2) |
| Exp D OE | MiniSAR + SAR-ship | SAR-ship만 | MiniSAR 비공개 → SAR-ship 대체 |
| Exp D OOD 테스트 | Holdout + MSTAR-O + MSTAR-P (Figure 9) | Holdout + MSTAR-O + SAR-ship(보조) | MSTAR-P 소스 특정 불가로 우선순위 낮춤, SAR-ship을 보조 far-OOD로 대체 |
| Exp D ID | SAMPLE, K=0.1 | SAMPLE, K=0.1 + 3단계 대비 증강 | 동일 |
...
| Exp C K=0 Ori 베이스라인 격차 (63.8% vs 논문 91.9%) | seed 1개뿐이라 분산 미확인, 원인 미조사 | seeds=[0,1,2]×4모델 확장 후 재조사(진행 중) |
| Exp D 분산 미확인 | seed=0 1회 검증만 완료 | seeds=[0,1,2] 확장(진행 중, Colab 런타임 반복 끊김으로 중단) |
| Exp D MSTAR-P 미구현 | 논문이 정확한 출처를 밝히지 않아 소스 특정 불가 | 우선순위 낮음(논문도 MSTAR-P가 MSTAR-O보다 쉽다고 명시) |
...
- **MSTAR-O**: Exp D far-OOD로 쓰는 별도 5클래스(BRDM2/BTR60/D7/T62/ZIL131) — Mixed Targets CD1/CD2 공식 raw에서 amplitude 렌더링으로 구성
...
- 논문에서는 MiniSAR과 함께 OE(Outlier Exposure) **학습 재료**로 쓰임 — 우리는 MiniSAR이 비공개라 대체하지 못했고, 대신 SAR-ship을 Exp D의 **보조 far-OOD 테스트셋**으로 활용(논문 Figure 9의 공식 OOD 테스트셋은 아님, Holdout/MSTAR-O/MSTAR-P임)
