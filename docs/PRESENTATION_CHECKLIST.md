...
| 개선 결과 | **XAI(개선#3)**: 어트리뷰션이 산란점 위치에 집중하는가(coverage/IoU) — "물리적으로 타당한 특징을 학습했다"는 근거. 해상도 단조증가 서사 — Grad-CAM 8×8(1.05×) → Grad-CAM 16×16(1.84×) → Occlusion 픽셀(3.21×, 인과) → **SmoothGrad-IG 픽셀(11.95×)**. 정성=Occlusion, 정량=SmoothGrad-IG, Grad-CAM 2단은 CAM 해상도 천장 대조 |
...
6. ✅ **Exp B XAI(개선#3)**: 90.9% 모델로 재실행 완료 — Grad-CAM 8×8 1.05× / Grad-CAM 16×16 1.84× / Occlusion 3.21× / SmoothGrad-IG **11.95×** (`run_gradcam_analysis` + `run_xai_analysis`, Cell 7b·7c)
