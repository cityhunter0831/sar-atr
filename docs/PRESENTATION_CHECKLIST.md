...
| 개선 결과 | **XAI(개선#3)**: 어트리뷰션이 산란점 위치에 집중하는가(coverage/IoU) — "물리적으로 타당한 특징을 학습했다"는 근거. Grad-CAM(1.05×)은 SMPL 8×8 특징맵 해상도 한계 → **픽셀 단위 XAI로 확장**: Occlusion 3.21×(인과) / **SmoothGrad-IG 11.95×**(정량, 산란점에 약 12배 집중). 정성=Occlusion, 정량=SmoothGrad-IG로 발표 |
...
6. ✅ **Exp B XAI(개선#3)**: 90.9% 모델로 재실행 완료 — Grad-CAM 1.05× / Occlusion 3.21× / SmoothGrad-IG **11.95×** (`run_xai_analysis`, Cell 7c)
