...
**Grad-CAM의 한계**: SMPL의 마지막 conv 특징맵이 **8×8**뿐이라 얇은 점 산란체를 국소화 못 함 → 비율 **1.05×**, IoU 0.11 (해상도 천장). CAM 계열의 구조적 한계. 한 단계 앞 conv(16×16)에서 뽑으면 1.84×로 개선되나, 여전히 픽셀 단위엔 못 미침.

**해상도를 올릴수록 산란점 정합이 단조 증가** (완전판 90.9% 모델, El15° 실측 10장 기준):

| 방법 | 성격 / 해상도 | 비율(coverage/기준선) | 평균 IoU |
|---|---|---|---|
| Grad-CAM | CAM 계열 (8×8, 표준) | 1.05× | 0.11 |
| Grad-CAM | CAM 계열 (16×16, `from_last=1`) | 1.84× | 0.17 |
| **Occlusion Sensitivity** | 인과적, 픽셀 공간 | **3.21×** | 0.24 |
| **SmoothGrad-IG** | 공리적, 픽셀 공간 | **11.95×** | 0.29 |

(산란점 추출을 타겟 영역으로 제한해 배경 스페클 오검출을 제거한 뒤 수치. 랜덤 기준선은 비율≈1.0×·IoU≈0.05.)
**서사**: 어트리뷰션 해상도가 8×8 → 16×16 → 픽셀 단위로 높아질수록 산란점 집중도가 단조 증가 → CAM 계열의 "해상도 천장"을 데이터로 입증하고, 픽셀 단위 XAI가 이를 극복함을 보임.

**지표 의미**:
- **비율(coverage/기준선)** = 산란점 위치 평균 어트리뷰션 ÷ 전체 평균. "산란점에 몇 배 집중하나" = 집중도 헤드라인.
- **IoU(Intersection over Union)** = 산란점 마스크 ∩ 상위 20% 어트리뷰션 영역 ÷ 합집합. 공간 위치 정합의 엄격 지표(배경을 켜면 페널티). 산란점은 5개 점, 어트리뷰션은 타겟 줄기 전체라 구조적으로 0.3~0.4가 상한 → 0.29는 강한 정합.

- **Occlusion**: 산란점 영역을 가리면 타깃 클래스 확률이 급락 → "그 영역이 분류에 인과적으로 기여"를 직접 입증.
- **SmoothGrad-IG**: 입력 픽셀 공간에서 직접 어트리뷰션 → CAM 해상도 천장 없음. **비율 11.95× = 모델 어트리뷰션이 이미지 평균보다 산란점에 약 12배 집중**. 물리 기반 증강(산란점 재합성)으로 학습한 모델이 실제로 산란점을 근거로 분류함을 정량 입증(우연 아님).
