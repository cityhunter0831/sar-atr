...
| 실험 | 논문 지표 | 우리 결과 | 판정 |
|---|---|---|---|
| Exp A Table 4 (클러터 전이) | 도메인 갭 → CT로 회복 | TrainOR→CT 하락(67.7%) / TrainCT 회복 **96.3%**(논문 91.5% 초과) / CTx2 40.1% ⚠️ 공개 데이터 3클래스 누락 | 🟢 핵심 재현 / ⚠️ CTx2 데이터 불완전 |
...
| TrainCTx2+TestCT | **40.1±0.1%** ⚠️ | **96.0±1.03%** | **41.1±0.0%** ⚠️ | **98.4±0.35%** |

- **핵심 서사 재현**: 원본 학습 → CT 테스트에서 하락(도메인 갭) → CT 학습으로 회복. **TrainCT+TestCT 96.3%(논문 91.5% 초과)**. 클러터 전이의 유효성 재현 완료.
- **CTx2 붕괴 원인 최종 확정 — 공개 데이터 불완전**: T5 진단 로그 결과:
  ```
  Train_CTx2_Test_CT: 1327장
  {'2s1': 847, 'bmp2': 480, 'btr70': 0, 't72': 0, 'zsu23': 0}
  ```
  **gengzhe2015 공개 데이터셋의 CTx2 폴더에 btr70·t72·zsu23이 0장**. 5클래스 중 3클래스가 학습 데이터에 없으니 테스트에서 그 3클래스를 전혀 못 맞춤 → 40% 붕괴. 코드 버그가 아니라 **저자 공개 데이터 자체가 불완전**한 것이 원인. BUG-3 수정(폴더 전체 사용)은 올바르게 적용됐으나 데이터 부재로 근본 해결 불가.
- **발표 처리**: "CTx2 조건은 공개 데이터 불완전으로 재현 불가(3/5 클래스 누락 확인). TrainCT 조건(1종 클러터 전이)에서 도메인 갭→회복 패턴 96.3%로 재현 완료." 정직하게 보고.
