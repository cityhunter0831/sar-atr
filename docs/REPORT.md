...
### 결론 및 논문 대비 비교 (1차 Colab 실행, RN18)
| 조건 | 우리 결과 | 논문/목표 |
|---|---|---|
| ① 증강 없음 (synth→measured) | 72.9% | 논문 Ori 91.9% |
| ② 논문 ColorJitter(0.5)×3 재현 | **61.0%** | 논문 Aug 94.5% |
| ③ Optuna 자동탐색 (strength=0.27, levels=4) | **79.2%** | ②보다 +18.2%p |

- **⭐ 핵심 발견 (개선 #2 서사)**: 논문의 매직넘버 `contrast=0.5`가 **이 데이터(SAMPLE dB)에선 오히려 성능을 해침**(72.9→61.0%). 대비를 과하게 흔들어 정보 손실. Optuna가 **훨씬 약한 strength=0.27**을 찾아 **61.0→79.2%(+18.2%p)** 회복. → "근거 없는 임의 대비값은 데이터에 따라 독이 될 수 있고, 자동탐색이 근거 있는 최적을 찾는다"를 정량 입증.
- **데이터 누수 방지 (2차 재실행)**: 1차는 Optuna가 test셋에서 선택하는 누수가 있어, real 평가셋을 **val(20%)/test(80%)로 분할** — Optuna는 real_val로만 선택, 최종 수치는 못 본 real_test로 보고하도록 수정(`_split_val_test`). 논문 0.5도 동일 test로 공정 비교. **Cell 8 재실행해 누수 없는 최종 수치로 갱신 필요**.
- **절대 수치 갭(72.9 vs 논문 91.9)**: 별도 이슈. SAMPLE 로드 장수(train/test 각 1345)가 논문 Table 5(806/539)와 달라 전처리·서브셋 규약 점검 필요(개선 #2 서사와는 무관).
