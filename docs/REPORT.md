...
| Exp B Table 3 (PH 보간, few-shot 136장) | 56.6% → 96.4% (SMPL/AT) | 선형보간: 49.8%→64.9% / **산란점 완전판: 55.9%→88.5%** (log-amp 60dB, AT) | 🟢 근접 재현 |
| Exp C Table6 (대비 증강, SAMPLE K=0) | 91.9%→94.5%(RN18) | ①no-aug 81.5% / ②논문 0.5×3 61.8% / ③Optuna(0.317×4) **75.4%** (+13.6%p) | 🟢 개선 확인 |
...
|---|---|---|---|---|
| MSTAROR (원본→원본) | 96.0±2.6% | 98.1±0.72% | 100.0±0.0% | 99.8±0.06% |
...
## Exp B — 위상이력(PH) 보간 증강 (논문 Table 2·3) ⭐ 근접 재현 (55.9%→88.5%)
...
|---|---|---|---|
| MSTAR-R (few-shot 136장, 증강 없음) | 49.8% | **55.9%** (log-amp 60dB) | 56.6% |
| MSTAR-Aug (PH 증강 추가) | 64.9% | **88.5%** (log-amp 60dB, AT) | **96.4%** |

- **Baseline(few-shot) 재현**: 완전판 55.9% vs 논문 56.6% — few-shot 설계(136장, 5클래스, 17°/15°)가 논문 난이도를 거의 동일하게 재현.
...
- **최종**: 88.5% vs 논문 96.4% (갭 7.9%p). few-shot·산란점 물리증강 파이프라인 전체를 로컬 MATLAB↔Colab 하이브리드로 완주하고 논문에 근접 재현. 잔여 갭은 합성-실측 도메인 차이 및 클래스별 σ_G 미세조정 미완에 기인.
...
| 조건 | 우리 결과 | 논문/목표 |
|---|---|---|
| ① 증강 없음 (synth→measured) | **81.5%** | 논문 Ori 91.9% |
| ② 논문 ColorJitter(0.5)×3 재현 | **61.8%** | 논문 Aug 94.5% |
| ③ Optuna 자동탐색 (strength=0.317, levels=4) | **75.4%** | ②보다 **+13.6%p** |

- **⭐ 핵심 발견 (개선 #2 서사)**: 논문의 매직넘버 `contrast=0.5`가 **이 데이터(SAMPLE)에선 오히려 성능을 크게 해침**(81.5→61.8%, -19.7%p). Optuna가 `strength=0.317, levels=4`를 찾아 **61.8→75.4%(+13.6%p)** 회복. → "근거 없는 임의 대비값은 데이터에 따라 독이 될 수 있고, 자동탐색이 근거 있는 최적을 찾는다"를 정량 입증.
- **누수 방지 적용**: real 평가셋을 **val(20%)/test(80%)로 분할** — Optuna는 real_val로만 파라미터 선택, 최종 수치는 못 본 real_test(1076장)로 보고. 공정 비교 확보.
- **절대 수치 갭(81.5 vs 논문 91.9)**: SAMPLE 로드 장수(train/test 각 1345)가 논문 Table 5(806/539)와 달라 서브셋 규약 차이로 인한 것. 개선 #2의 핵심 서사(+13.6%p 개선)와는 무관.
...
| 2 | **대비 Optuna 자동탐색** (Exp C) | `contrast=0.5`·3레벨 매직넘버 임의 고정 | Optuna로 strength·levels 자동탐색 → 근거 있는 최적값 | +13.6%p (61.8→75.4%) |
...
**해상도를 올릴수록 산란점 정합(IoU)이 단조 증가** (완전판 88.5% 모델, El15° 실측 10장, 산란점 추출을 타겟 영역으로 제한한 뒤 수치):
...
| Exp B | PH 보간 few-shot, Agarwal MATLAB (Table 3) | 원본 MATLAB 하이브리드로 직접 실행, log-amp 60dB/AT → **88.5%** + **XAI 산란점 검증(개선#3)** | 근접 재현 (88.5% vs 96.4%) + 해석가능성 추가 | 🟢 |
| Exp C | ColorJitter(contrast=0.5)×3 매직넘버 고정 | 논문 방법 재현(baseline) → **Optuna 자동탐색(개선#2)** → +13.6%p | 재현 + 방법론적 개선 | 🟢 |
